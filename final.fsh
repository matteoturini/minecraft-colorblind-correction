...
The original shader is taken from https://godotshaders.com/shader/colorblindness-correction-shader/
...
- Modified on 16/05/2022 by 8x14b porting it to a Minecraft OptiFine shader
...
void main()
{
	vec4 tex = texture2D(gcolor, texcoord);
	...
	float l, m, s;
		...
		m = 0.0 * L + 1.0 * M + 0.0 * S;
		s = 0.0 * L + 0.0 * M + 1.0 * S;
  #elif mode == 1 // Deutranopia
		l = 1.0 * L + 0.0 * M + 0.0 * S;
    m = 0.494207 * L + 0.0 * M + 1.24827 * S;
    s = 0.0 * L + 0.0 * M + 1.0 * S;
  #elif mode == 2 // Tritanopia
		l = 1.0 * L + 0.0 * M + 0.0 * S;
    m = 0.0 * L + 1.0 * M + 0.0 * S;
